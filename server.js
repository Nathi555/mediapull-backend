...
app.post('/api/download', async (req, res) => {
  ...
  let args;
  if (isAudio) {
    args = `-x --audio-format ${audioFormat} --audio-quality 0 --extractor-args "youtube:player_client=android,web,tv_embedded" --js-runtimes nodejs ${cookies} -o "${outFile}" --no-playlist "${url}"`;
  } else {
    ...
    args = `-f "${fmt}" --merge-output-format mp4 --extractor-args "youtube:player_client=android,web,tv_embedded" --js-runtimes nodejs ${cookies} -o "${outFile}" --no-playlist "${url}"`;
  }
